...
const fallbackVehicles = [
  { id: 1, name: 'Toyota Fortuner', type: 'SUV', seats: 7, rating: 4.9, price_per_day: 5200, image_url: '/images/vehicles/toyota_fortuner.jpg', emoji: '🚙' },
  { id: 2, name: 'Honda City', type: 'Sedan', seats: 5, rating: 4.7, price_per_day: 3600, image_url: '/images/vehicles/hondacity.jpg', emoji: '🚘' },
  { id: 3, name: 'Tata Nexon', type: 'SUV', seats: 5, rating: 4.6, price_per_day: 4100, image_url: '/images/vehicles/nexon.jpg', emoji: '🚗' },
  { id: 4, name: 'Swift Zip', type: 'Hatchback', seats: 5, rating: 4.5, price_per_day: 2400, image_url: '/images/vehicles/swift.jpg', emoji: '🚕' },
  { id: 5, name: 'Pulsar Rider', type: 'Bike', seats: 2, rating: 4.8, price_per_day: 1400, image_url: '/images/vehicles/pulsar.jpg', emoji: '🏍️' },
  { id: 6, name: 'Duke Street', type: 'Bike', seats: 2, rating: 4.4, price_per_day: 1700, image_url: '/images/vehicles/duke.jpg', emoji: '🏍️' }
];
...
function showToast(message, type = 'success') {
  ...
  gsap.fromTo(toast, { x: 80, opacity: 0 }, { x: 0, opacity: 1, duration: 0.35, ease: 'power3.out' });
  setTimeout(() => gsap.to(toast, { x: 80, opacity: 0, duration: 0.3, onComplete: () => toast.remove() }), 3500);
}
...
async function handleApiError(err, options = {}) {
  if (err.status === 401) {
    if (options.authRequiredMessage) showToast(options.authRequiredMessage, 'info');
...
function splitHeroChars() {
  document.querySelectorAll('.hero-line').forEach((line) => {
    ...
    line.innerHTML = txt.split('').map((ch) => `<span class="hero-char">${ch === ' ' ? '&nbsp;' : ch}</span>`).join('');
  });
}

function animateHero() {
  gsap.from('.hero-char', { y: 80, opacity: 0, duration: 0.8, stagger: 0.03, ease: 'power4.out', delay: 0.3 });
}

function initScrollReveals() {
  gsap.utils.toArray('.reveal').forEach((item) => {
    gsap.from(item, { y: 36, opacity: 0, duration: 0.65, ease: 'power3.out', scrollTrigger: { trigger: item, start: 'top 84%' } });
  });
}
...
function initCounters() {
  document.querySelectorAll('[data-counter]').forEach((node) => {
    ...
    gsap.to(obj, {
      ...
      onUpdate: () => { node.textContent = decimal ? (obj.v / 10).toFixed(1) : `${Math.floor(obj.v)}+`; }
    });
  });
}

function initMarquee() {
  const part = 'INSTANT BOOKING <span class="dot">✦</span> FREE CANCELLATION <span class="dot">✦</span> 24/7 SUPPORT <span class="dot">✦</span> FULL INSURANCE <span class="dot">✦</span> PREMIUM FLEET <span class="dot">✦</span>';
  document.getElementById('marqueeTrack').innerHTML = `<div class="marquee-item">${part}</div><div class="marquee-item">${part}</div><div class="marquee-item">${part}</div><div class="marquee-item">${part}</div>`;
}

function renderSkeletons() {
  el.vehicleGrid.innerHTML = '<article class="skeleton-card"></article><article class="skeleton-card"></article><article class="skeleton-card"></article>';
}
...
function resolveVehicleImage(imageUrl = '', vehicleName = '') {
  const nameKey = String(vehicleName || '').trim().toLowerCase();
  const forcedByName = {
...
function applyQuickFilter(list) {
  if (quickFilter === 'car') return list.filter((v) => String(v.type).toLowerCase() === 'car');
  if (quickFilter === 'bike') return list.filter((v) => String(v.type).toLowerCase() === 'bike');
  ...
  try {
    ...
    if (hasDateRange) {
      ...
      const res = await api('/vehicles/available', {
        method: 'POST',
        body: JSON.stringify({ start_date: filterVals.fromDate, end_date: filterVals.toDate })
      });
      rows = res.data || [];
    } else {
    ...
    vehicles = normalized;
    filteredVehicles = hasDateRange ? applyLocalFilters(normalized, filterVals) : applyQuickFilter(normalized);
    renderVehicleGrid();
  } catch (err) {
...
function renderVehicleGrid() {
  ...
  if (!filteredVehicles.length) {
    el.vehicleGrid.innerHTML = '<div class="empty"><span class="emoji">🔍</span>No vehicles matched your filters. Try broader search.</div>';
    return;
  }
  el.vehicleGrid.innerHTML = filteredVehicles.map(vehicleCardTemplate).join('');
  setupCardImageLoading();
  gsap.from(el.vehicleGrid.querySelectorAll('.vehicle-card'), { y: 40, opacity: 0, duration: 0.5, stagger: 0.08, ease: 'power3.out' });
}
...
function setActivePill(pill) {
  quickFilter = pill;
  document.querySelectorAll('.pill').forEach((b) => b.classList.toggle('active', b.dataset.pill === pill));
...
async function switchView(view) {
  ...
  if (view === 'bookings') { document.querySelector('.nav-link[data-link="bookings"]').classList.add('active'); await loadBookings(); }
  if (view === 'payments') { document.querySelector('.nav-link[data-link="payments"]').classList.add('active'); await loadPayments(); }
...
function openModal(id) {
  ...
  gsap.fromTo(modal, { scale: 0.95, opacity: 0 }, { scale: 1, opacity: 1, duration: 0.25, ease: 'power2.out' });
}

function closeModal(id) {
  const overlay = document.getElementById(id);
  const modal = overlay.querySelector('.modal');
  gsap.to(modal, { scale: 0.95, opacity: 0, duration: 0.18, onComplete: () => overlay.classList.remove('active') });
}
...
function openBookingModal(vehicleId) {
  ...
  document.getElementById('dailyRate').textContent = formatMoney(vehicle.price_per_day);
...
function successAnimation(targetEl, message, sub = '') {
  ...
  gsap.fromTo(circle, { scale: 0 }, { scale: 1, duration: 0.45, ease: 'back.out(2)' });
  gsap.to(path, { strokeDashoffset: 0, duration: 0.45, delay: 0.12, ease: 'power2.out' });
  for (let i = 0; i < 7; i += 1) {
    const c = document.createElement('span');
    c.style.position = 'absolute'; c.style.left = '50%'; c.style.top = '5px'; c.style.width = '8px'; c.style.height = '8px';
    c.style.background = ['#ff3c00', '#ffd166', '#00e5ff', '#00e676'][i % 4]; c.style.transform = 'translate(-50%, -50%)';
    targetEl.querySelector('.confetti-host').appendChild(c);
    gsap.to(c, { x: gsap.utils.random(-90, 90), y: gsap.utils.random(-40, 50), rotation: gsap.utils.random(-240, 240), opacity: 0, duration: 0.8, ease: 'power2.out', onComplete: () => c.remove() });
  }
}
...
function openPaymentModal(bookingId, amount) {
  ...
  selectedMethod = 'CARD';
  document.querySelectorAll('.method-card').forEach((card) => card.classList.toggle('active', card.dataset.method === 'CARD'));
...
async function loadBookings() {
  ...
  try {
    ...
    const list = res.data || [];
    if (!list.length) {
      el.bookingList.innerHTML = '<div class="empty"><span class="emoji">🧳</span><h3>No trips yet</h3><p>Your confirmed bookings will appear here.</p></div>';
      return;
    }
...
async function loadPayments() {
  ...
  try {
    ...
    const list = res.data || [];
    if (!list.length) {
      el.paymentList.innerHTML = '<div class="empty"><span class="emoji">💳</span><h3>No payments yet</h3><p>Completed transactions will be listed here.</p></div>';
      return;
    }
    const rows = list.map((p) => `<tr><td>${escapeHtml(p.transaction_id || '-')}</td><td>${escapeHtml(p.vehicle_name || '-')}</td><td>${fmtDate(p.start_date)} - ${fmtDate(p.end_date)}</td><td>${formatMoney(p.amount)}</td><td>${escapeHtml(p.payment_method || '-')}</td><td><span class="status ${String(p.status || 'PAID').toLowerCase()}">${p.status || 'PAID'}</span></td></tr>`).join('');
    ...
    if (await handleApiError(err, { authRequiredMessage: 'Login required to view payments' })) return;
  }
}
...
async function handleSignup(e) {
  ...
  try {
    const res = await api('/auth/signup', { method: 'POST', body: JSON.stringify({ name, email, phone, password }) });
...
function setupAuthTabs() {
  document.querySelectorAll('.auth-tab').forEach((tab) => {
    tab.addEventListener('click', () => {
      ...
      document.getElementById('authTabs').style.setProperty('--tab-x', which === 'signup' ? '100%' : '0%');
...
function initParticles() {
  ...
  const particles = Array.from({ length: 72 }, (_, i) => ({ x: Math.random() * w, y: Math.random() * h, vx: (Math.random() - 0.5) * 0.35, vy: (Math.random() - 0.5) * 0.35, r: 1.5 + Math.random() * 1.5, o: 0.15 + Math.random() * 0.25, glow: i < 2 }));
  function draw() {
    ...
    for (let i = 0; i < particles.length; i += 1) {
      ...
      ctx.beginPath();
      ctx.fillStyle = p.glow ? 'rgba(255,60,0,0.6)' : `rgba(255,255,255,${p.o})`;
...
function initCursor() {
  ...
  let mx = window.innerWidth / 2; let my = window.innerHeight / 2;
  let rx = mx; let ry = my;
...
function bindEvents() {
  document.getElementById('browseFleetBtn').addEventListener('click', async () => { await switchView('home'); document.getElementById('vehicles').scrollIntoView({ behavior: 'smooth' }); });
  document.getElementById('howItWorksBtn').addEventListener('click', async () => { await switchView('home'); document.getElementById('features').scrollIntoView({ behavior: 'smooth' }); });
  el.mobileToggle.addEventListener('click', () => el.navLinks.classList.toggle('open'));

  document.querySelectorAll('.nav-link').forEach((btn) => btn.addEventListener('click', async () => {
    const key = btn.dataset.link;
    el.navLinks.classList.remove('open');
    if (key === 'fleet') { await switchView('home'); document.getElementById('vehicles').scrollIntoView({ behavior: 'smooth' }); }
    else if (key === 'features') { await switchView('home'); document.getElementById('features').scrollIntoView({ behavior: 'smooth' }); }
    else if (key === 'bookings') await switchView('bookings');
    else if (key === 'payments') await switchView('payments');
  }));

  document.querySelectorAll('[data-footer]').forEach((link) => link.addEventListener('click', async (e) => {
    e.preventDefault();
  ...
  el.signupBtn.addEventListener('click', () => { openModal('authOverlay'); document.querySelector('.auth-tab[data-auth-tab="signup"]').click(); });
  el.logoutBtn.addEventListener('click', handleLogout);

  document.querySelectorAll('[data-close]').forEach((btn) => btn.addEventListener('click', () => closeModal(btn.dataset.close)));
  el.authOverlay.addEventListener('click', (e) => { if (e.target === el.authOverlay) closeModal('authOverlay'); });
  el.bookingOverlay.addEventListener('click', (e) => { if (e.target === el.bookingOverlay) closeModal('bookingOverlay'); });
  el.paymentOverlay.addEventListener('click', (e) => { if (e.target === el.paymentOverlay) closeModal('paymentOverlay'); });
  ...
  window.addEventListener('storage', async (e) => {
    if (e.key === 'drivex_booking_updated' || e.key === 'drivex_booking_cancelled') {
      await syncFleet();
    }
  });

  document.querySelectorAll('.pill').forEach((btn) => btn.addEventListener('click', () => setActivePill(btn.dataset.pill)));

  el.vehicleGrid.addEventListener('click', (e) => {
    const likeBtn = e.target.closest('[data-like]');
    if (likeBtn) {
      const id = Number(likeBtn.dataset.like);
      if (likedVehicles.has(id)) likedVehicles.delete(id); else likedVehicles.add(id);
      renderVehicleGrid();
      gsap.fromTo(`[data-like="${id}"]`, { scale: 0.8 }, { scale: 1, duration: 0.28, ease: 'back.out(3)' });
      return;
    }
  ...
  document.getElementById('bookingStart').addEventListener('change', () => {
    document.getElementById('bookingEnd').min = document.getElementById('bookingStart').value || todayIso();
    updateBookingBreakdown();
  });
  document.getElementById('bookingEnd').addEventListener('change', updateBookingBreakdown);

  document.getElementById('confirmBookingBtn').addEventListener('click', async () => {
    if (!currentUser) { showToast('Please login to continue booking', 'info'); openModal('authOverlay'); return; }
    const start_date = document.getElementById('bookingStart').value;
    const end_date = document.getElementById('bookingEnd').value;
    if (!start_date || !end_date || new Date(start_date) >= new Date(end_date)) { showToast('Please select a valid date range', 'error'); return; }
    try {
      const res = await api('/bookings', { method: 'POST', body: JSON.stringify({ vehicle_id: currentVehicle.id, start_date, end_date }) });
  ...
  el.payNowBtn.addEventListener('click', async () => {
    ...
    if (!currentBookingId) { showToast('No booking selected for payment', 'error'); return; }
    ...
    try {
      const res = await api('/payments', { method: 'POST', body: JSON.stringify({ booking_id: currentBookingId, payment_method: selectedMethod, amount: bookingTotal }) });
      successAnimation(el.paymentSuccess, 'Payment Successful', `Transaction: ${res.transaction_id || 'TXN'}`);
      showToast('Payment completed', 'success');
      setTimeout(() => { closeModal('paymentOverlay'); if (activeView === 'bookings') loadBookings(); if (activeView === 'payments') loadPayments(); }, 1800);
    } catch (err) {
      await handleApiError(err, { authRequiredMessage: 'Login required for payment' });
    } finally {
  ...
  el.bookingList.addEventListener('click', async (e) => {
    ...
    if (cancelBtn) {
      try {
        ...
        await handleApiError(err, { authRequiredMessage: 'Login required to cancel booking' });
      }
    }
  });
}
